...
import { incrementVegCount, incrementNonVegCount } from '../features/countSlice'
...
const HomeTabScreen = () => {
    ...
    const handleAddVeg = () => {
        if (activeCounter === null) {
            dispatch(incrementVegCount())
            setActiveCounter('veg')
        }
    }

    const handleAddNonVeg = () => {
        if (activeCounter === null) {
            dispatch(incrementNonVegCount())
            setActiveCounter('nonVeg')
        }
    }
        ...
        <View style={styles.container}>
            ...
            <View style={styles.buttonContainer}>
                <TouchableWithoutFeedback onPress={handleAddVeg}>
                    ...
                    >
                        <LinearGradient
                            colors={['#0dbfb1', '#19635d']}
                            style={styles.gradient}
                        >
                            <View style={styles.buttonContent}>
                                {activeCounter === 'veg' ? (
                                    <LottieView
                                        source={require('../assets/AnimationTickWColor.json')}
                                        autoPlay
                                        loop={false}
